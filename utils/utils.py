...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            ax.plot(np.concatenate(([0.], recall)),
                    np.concatenate(([0.], precision)))
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:  # number of targets
            ...
            pbox = torch.cat((pxy, torch.exp(ps[:, 2:4]).clamp(
                max=1E3) * anchor_vec[i]), 1)  # predicted box
            # giou computation
            giou = 1.0 - bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, GIoU=True)
...
def kmeans_targets(path='../coco/trainvalno5k.txt', n=9, img_size=416):
    ...
    for s, l in zip(dataset.shapes, dataset.labels):
        ...
        l[:, 1:] *= img_size / max(s) * random.uniform(0.5, 1.5)
...
def fitness(x):
    ...
    # weighted combination of x=[p, r, mAP@0.5, F1 or mAP@0.5:0.95]
    return x[:, 2] * 0.1 + x[:, 3] * 0.9
...
def rand_color(idx=None):
    color_list = ['b', 'g', 'r', 'y']
    if idx is None:
        select_index = random.randint(0, len(color_list)-1)
    else:
        select_index = idx
    return color_list[select_index]


def rand_marker(idx=None):
    marker_list = ['.-', 'x-', '+-', '*-', '1-', '2-', '3-']
    if idx is None:
        select_index = random.randint(0, len(marker_list)-1)
    else:
        select_index = idx
...
def plot_map(start=0, stop=0, bucket='', id=()):  # from utils.utils import *; plot_map()
    ...
    for i, f in enumerate(sorted(files)):
        ...
        x = range(start, min(stop, n) if stop else n)
        map_results = results[8]

        plt.plot(x, map_results[x], rand_color(
            i)+rand_marker(), label=label_name)  # ,markersize=5)

    plt.xlabel("Epoch")
    plt.ylabel("Average validation mAP")
    ...
    plt.savefig('results_map.png', dpi=200)


def plot_loss(start=0, stop=0, bucket='', id=()):  # from utils.utils import *; plot_results()
    from matplotlib.ticker import FuncFormatter
    plt.figure(figsize=(10, 5))
    plt.style.use('ggplot')
    ...
    for i, f in enumerate(sorted(files)):
        ...
        x = range(start, min(stop, n) if stop else n)
        x_max = max(x_max, n)
        map_results = results[5] + results[6] + results[7]
